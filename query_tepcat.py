...
def get_page(planet):
	...
	html = urllib.request.urlopen(query_url).read()
	soup = BeautifulSoup(html, 'html.parser')
	return soup


# os.system('wget http://www.astro.keele.ac.uk/jkt/tepcat/allplanets-ascii.txt')
planets = np.loadtxt('data/TEPcat_allplanets-ascii.txt',skiprows=0,usecols=0,dtype='str')
# planets = np.loadtxt('data/TEPcat_allplanets.csv',skiprows=1,usecols=0,dtype='str')

lines = open('data/TEPcat_allplanets-ascii.txt').readlines()
# lines = open('data/TEPcat_allplanets.csv').readlines()
line0 = lines[0]
lines = lines[1:]
i=0
for line in planets:
